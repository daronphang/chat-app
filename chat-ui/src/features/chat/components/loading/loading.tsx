...
import { setWsUrl } from 'core/config/configSlice';
...
export default function Loading({ handleLoading, handleAlert }: LoadingProps) {
  ...
  const fetchWsUrl = async () => {
    try {
      ...
      const resp = await api.USER_SERVICE.getBestServer(payload);
      dispatch(setWsUrl(resp.getMessage()));
    } catch (e) {
      const err = e as RpcError;
      if (err.code === 14) {
        enqueueSnackbar(ApiErrors.NETWORK_ERROR, {
